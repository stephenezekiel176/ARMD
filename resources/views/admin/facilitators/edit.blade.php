...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Facilitator</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Update facilitator information</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <form method="POST" action="{{ route('admin.facilitators.update', $facilitator->id) }}" class="space-y-6 p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" 
                               id="fullname" 
                               name="fullname" 
                               value="{{ old('fullname', $facilitator->fullname) }}"
                               required
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               required
                               placeholder="e.g. Senior Facilitator, Training Manager"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-primary-500 focus:ring-primary-500">
                        @error('position')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        ...
                        <select id="department_id" 
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" 
                                        {{ old('department_id', $facilitator->department_id) == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <!-- Statistics -->
            <div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Facilitator Statistics</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Courses</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $facilitator->courses_count ?? 0 }}</div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Personnel</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $facilitator->department_users_count ?? 0 }}</div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Member Since</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $facilitator->created_at->format('M Y') }}</div>
                    </div>
                </div>
            </div>
