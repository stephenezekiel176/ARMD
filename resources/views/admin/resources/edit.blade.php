...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Company Training</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Update training materials for all personnel</p>
            </div>
    ...
    <!-- Form -->
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <form action="{{ route('admin.resources.update', $resource) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="type" required
                    ...
                    <option value="ebook" {{ old('type', $resource->type) === 'ebook' ? 'selected' : '' }}>Ebook/Document</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Current File:</p>
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ basename($resource->file_path) }}</p>
            </div>

            <!-- File Upload (Optional for edit) -->
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Replace Training File (optional)
                </label>
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    Leave empty to keep current file. Accepted: Videos (MP4, AVI, etc.) or Documents (PDF, EPUB, DOC). Max size: 100MB
                </p>
                @error('file')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="description" rows="4" required
                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500"
                          placeholder="Describe what personnel will learn from this training...">{{ old('description', $resource->description) }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="department_id" required
                    ...
                    @foreach($departments as $department)
                        <option value="{{ $department->id }}" {{ old('department_id', $resource->department_id) == $department->id ? 'selected' : '' }}>
                            {{ $department->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="facilitator_id" required
                    ...
                    @foreach($facilitators as $facilitator)
                        <option value="{{ $facilitator->id }}" {{ old('facilitator_id', $resource->facilitator_id) == $facilitator->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" name="duration" value="{{ old('duration', $resource->duration) }}" min="1" required
                       class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500"
                       placeholder="e.g., 30">
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_previewable" id="is_previewable" value="1" {{ old('is_previewable', $resource->is_previewable) ? 'checked' : '' }}
