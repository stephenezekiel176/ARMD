...
<div class="max-w-4xl mx-auto">
    <form method="POST" action="{{ route('admin.symposiums.update', $symposiums) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $symposiums->title) }}" required
            ...
            <div>
                ...
                <textarea name="excerpt" rows="3"
                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">{{ old('excerpt', $symposiums->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="content" rows="15" required
                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">{{ old('content', $symposiums->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            @if($symposiums->featured_image)
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Current Featured Image
                </label>
                <img src="{{ Storage::url($symposiums->featured_image) }}" alt="Featured" class="w-48 h-32 object-cover rounded-lg">
            </div>
            @endif
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    {{ $symposiums->featured_image ? 'Replace Featured Image' : 'Featured Image' }}
                </label>
            ...
            @if($symposiums->images && count($symposiums->images) > 0)
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Current Additional Images
                </label>
                <div class="grid grid-cols-4 gap-4">
                    @foreach($symposiums->images as $index => $image)
                    <div class="relative">
                        <img src="{{ Storage::url($image) }}" alt="Image {{ $index + 1 }}" class="w-full h-24 object-cover rounded-lg">
                        <form action="{{ route('admin.symposiums.image.delete', [$symposiums, $index]) }}" method="POST" class="absolute top-1 right-1">
                            @csrf
                            @method('DELETE')
            ...
            <!-- Videos -->
            <div x-data="{ videos: {{ json_encode(old('videos', $symposiums->videos ?? [''])) }} }">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Video URLs
                </label>
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $symposiums->category) }}" list="categories"
                       class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">
            ...
            <!-- Tags -->
            <div x-data="{ tags: {{ json_encode(old('tags', $symposiums->tags ?? [])) }}, tagInput: '' }">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Tags
                </label>
            ...
            <div>
                ...
                <select name="status" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">
                    <option value="draft" {{ old('status', $symposiums->status) === 'draft' ? 'selected' : '' }}>Draft</option>
                    <option value="published" {{ old('status', $symposiums->status) === 'published' ? 'selected' : '' }}>Published</option>
                    <option value="archived" {{ old('status', $symposiums->status) === 'archived' ? 'selected' : '' }}>Archived</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="datetime-local" name="published_at" 
                       value="{{ old('published_at', $symposiums->published_at ? $symposiums->published_at->format('Y-m-d\TH:i') : '') }}"
                       class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">
                @error('published_at')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="font-medium text-gray-900 dark:text-white ml-2">{{ number_format($symposiums->views) }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Created:</span>
                        <span class="font-medium text-gray-900 dark:text-white ml-2">{{ $symposiums->created_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
