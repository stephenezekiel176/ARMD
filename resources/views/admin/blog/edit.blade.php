        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="excerpt" rows="3"
                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">{{ old('excerpt', $blog->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="content" rows="15" required
                          class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">{{ old('content', $blog->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <img src="{{ Storage::url($blog->featured_image) }}" alt="Featured" class="w-48 h-32 object-cover rounded-lg">
            </div>
            @endif
            ...
            <div>
                ...
                <div class="grid grid-cols-4 gap-4">
                    ...
                    <div class="relative">
                        ...
                        <form action="{{ route('admin.blog.image.delete', [$blog, $index]) }}" method="POST" class="absolute top-1 right-1">
                            @csrf
                            @method('DELETE')
            ...
            <!-- Videos -->
            <div x-data="{ videos: {{ json_encode(old('videos', $blog->videos ?? [''])) }} }">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Video URLs
                </label>
            ...
            <div>
                ...
                <input type="text" name="category" value="{{ old('category', $blog->category) }}" list="categories"
                       class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-primary-500 focus:ring-primary-500">
            ...
            <!-- Tags -->
            <div x-data="{ tags: {{ json_encode(old('tags', $blog->tags ?? [])) }}, tagInput: '' }">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    Tags
                </label>
                <div class="flex flex-wrap gap-2 mb-2">
                    <template x-for="(tag, index) in tags" :key="index">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-primary-100 text-primary-800 dark:bg-primary-900 dark:text-primary-200">
                            ...
                            <button type="button" @click="tags.splice(index, 1)" class="ml-2 text-primary-600 hover:text-primary-800">×</button>
                        </span>
                    </template>
                </div>
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="font-medium text-gray-900 dark:text-white ml-2">{{ number_format($blog->views) }}</span>
                    </div>
                    <div>
                        <span class="text-gray-500 dark:text-gray-400">Created:</span>
                        <span class="font-medium text-gray-900 dark:text-white ml-2">{{ $blog->created_at->format('M d, Y') }}</span>
                    </div>
                </div>
            </div>
