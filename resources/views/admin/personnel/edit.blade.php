...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        <div class="flex items-center">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Edit Personnel</h1>
                <p class="text-gray-600 dark:text-gray-400 mt-1">Update personnel information</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
        <form method="POST" action="{{ route('admin.personnel.update', $personnel->id) }}" class="space-y-6 p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" 
                               id="fullname" 
                               name="fullname" 
                               value="{{ old('fullname', $personnel->fullname) }}"
                               required
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="position" 
                               name="position" 
                               value="{{ old('position', $personnel->position) }}"
                               required
                        ...
                        <select id="department_id" 
                            ...
                            @foreach($departments as $department)
                                <option value="{{ $department->id }}" 
                                        {{ old('department_id', $personnel->department_id) == $department->id ? 'selected' : '' }}>
                                    {{ $department->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <!-- Statistics -->
            <div>
                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Personnel Statistics</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Enrollments</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $personnel->enrollments_count ?? 0 }}</div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Submissions</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $personnel->submissions_count ?? 0 }}</div>
                    </div>
                    <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
                        <div class="text-sm text-gray-500 dark:text-gray-400">Member Since</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $personnel->created_at->format('M Y') }}</div>
                    </div>
                </div>
            </div>
